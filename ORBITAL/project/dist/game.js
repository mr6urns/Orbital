...
function createStarfield() {
    ...
    for (let i = 0; i < starCount; i++) {
        const i3 = i * 3;
        const radius = 500; // Fixed radius for all stars
...
function generatePlanet(center, radius, isMoon = false) {
    ...
    const coreGeometry = new THREE.SphereGeometry(radius - hexHeight, 32, 32);
    const coreMaterial = new THREE.MeshPhongMaterial({
...
function updatePlayer(delta) {
    ...
    if (keys[' '] && jetpackEnergy > 0 && jetpackActive) {
        ...
        // Update jetpack glow effects
        const leftJetpack = document.querySelector('.jetpack-glow.left');
        const rightJetpack = document.querySelector('.jetpack-glow.right');
        
        if (leftJetpack && rightJetpack) {
            leftJetpack.classList.add('active');
            rightJetpack.classList.add('active');
            
            // Position the jetpack flames relative to the player's hands
            const handOffset = 8; // Horizontal offset from center
            const verticalOffset = 8; // Vertical offset from center
            
            leftJetpack.style.left = `calc(50% - ${handOffset}px)`;
            rightJetpack.style.left = `calc(50% + ${handOffset}px)`;
            
            leftJetpack.style.transform = `translate(-50%, ${verticalOffset}px)`;
            rightJetpack.style.transform = `translate(-50%, ${verticalOffset}px)`;
        }
    } else {
        ...
        // Hide jetpack glow when not active
        const leftJetpack = document.querySelector('.jetpack-glow.left');
        const rightJetpack = document.querySelector('.jetpack-glow.right');
        
        if (leftJetpack && rightJetpack) {
            leftJetpack.classList.remove('active');
            rightJetpack.classList.remove('active');
        }
    }
